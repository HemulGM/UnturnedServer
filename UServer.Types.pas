...
begin
  Result := '';
  if FromCity + FromCountry = '' then
    Exit;
  if (FromCity = '') or (FromCountry = '') then
    Result := FromCountry + FromCity
  else
    Result := FromCountry + ', ' + FromCity;
end;
...
begin
  ...
  if Count <= 0 then
    Exit;
  if not Between(0, FCurIndex - 1, (Count - 1)) then
    Exit(False);
  Dec(FCurIndex);
  try
...
begin
  if FAVGTMP <> 0 then
    AvgPlayTime := (AvgPlayTime div FAVGTMP);
  FAVGTMP := 0;
end;

function TCurrentStatistics.GetMaxPlayers: Cardinal;
begin
  if not IsToday(FMaxPlayersDate) then
    FMaxPlayers := 0;
  Result := FMaxPlayers;
end;
...
begin
  ...
  begin
    ...
    if PI.TotalPlayTime < EndPlayersTime then
  ...
  begin
    ...
    if PI.TotalPlayTime < EndPlayersTime then
      EndPlayersYest := EndPlayersYest + 1;
  end;
...
begin
  ...
  if Count <= 0 then
    Exit;
  Inc(FCurrentMessage);
  if FCurrentMessage > Count - 1 then
    FCurrentMessage := 0;
...
begin
  Result.ID := -1;
  if not IndexInList(Value, TList(Self)) then
    Exit;
